    ...
    CKEDITOR.plugins.add( 'footnotes', {
        ...
        init: function(editor) {
            ...
            var contents = $('<div>' + editor.element.$.textContent + '</div>')
                     ...
                     , i = 1;
            for (i; i <= l; i++) {
                def['footnote_' + i] = {selector: '#footnote' + prefix + '-' + i + ' cite', allowedContent: 'a[href]; cite[*](*); strong em i b span br'};
            }
            ...
            var header_el_names = header_els[0].replace(/</g, '');
            var header_el_names = header_el_names.replace(/>/g, '[*](*);');

            // Define an editor command that opens our dialog.
            editor.addCommand('footnotes', new CKEDITOR.dialogCommand('footnotesDialog', {
                // @TODO: This needs work:
                allowedContent: 'section[*](*);header[*](*);li[*];a[*];cite(*)[*];sup[*];' + header_el_names,
                requiredContent: 'section[*](*);header[*](*);li[*];a[*];cite(*)[*];sup[*];' + header_el_names
            }));
        ...
        reorderMarkers: function(editor) {
            ...
            for (i in data.order) {
                n = parseInt(i) + 1;
                footnote_widget.initEditable('footnote_' + n, {selector: '#footnote' + prefix + '-' + n +' cite', allowedContent: 'a[href]; cite[*](*); em strong i b span'});
            }
